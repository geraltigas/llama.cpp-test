...
void ggml_cl_init(void) {
    ...
    program = build_program_from_source(_global_context,_global_device, kernel_src.c_str());

    add_kernel("dequantize_block_q5_K",clCreateKernel(program, "dequantize_block_q5_K", &err));
    add_kernel("dequantize_block_q6_K",clCreateKernel(program, "dequantize_block_q6_K", &err));
    add_kernel("dequantize_mul_mat_vec_q5_K",clCreateKernel(program, "dequantize_mul_mat_vec_q5_K", &err));
    add_kernel("dequantize_mul_mat_vec_q6_K",clCreateKernel(program, "dequantize_mul_mat_vec_q6_K", &err));
    add_kernel("mul_f32",clCreateKernel(program, "mul_f32", &err));
}
...
static cl_kernel ggml_get_to_fp32_cl(ggml_type type) {
    switch (type) {
        ...
        case GGML_TYPE_Q5_K:
            return get_kernel("dequantize_block_q5_K");
        case GGML_TYPE_Q6_K:
...
static cl_kernel ggml_get_dequantize_mul_mat_vec_cl(ggml_type type) {
    switch (type) {
        ...
        case GGML_TYPE_Q5_K:
            return get_kernel("dequantize_mul_mat_vec_q5_K");
        case GGML_TYPE_Q6_K:
            return get_kernel("dequantize_mul_mat_vec_q6_K");
        default:
            return nullptr;
    }
}
...
static cl_mem ggml_cl_pool_malloc(size_t size) {
    ...
    return clCreateBuffer(_global_context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_WRITE, size, buffer_mem, &err);
}
...
static cl_mem ggml_cl_pool_malloc(size_t size, size_t * actual_size) {
    ...
    cl_mem mem;
    CL_CHECK((mem = clCreateBuffer(_global_context, CL_MEM_READ_WRITE | CL_MEM_ALLOC_HOST_PTR, size, NULL, &err), err));
    *actual_size = size;
    return mem;
}

static cl_mem ggml_cl_pool_malloc_with_unified_mem(size_t size, void* ptr) {
    cl_int err;
    ...
    //     b.size = 0;
    //     clReleaseMemObject(mem);
    cl_mem mem = clCreateBuffer(_global_context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_WRITE, size, ptr, &err);
    if (err != CL_SUCCESS) {
        fprintf(stderr, "ggml_opencl: clCreateBuffer error %d at %s:%d\n", err, __FILE__, __LINE__);
        // get the error string
        size_t len;
...
static cl_mem ggml_cl_h2d_tensor_2d_with_unified_mem(const ggml_tensor * src, size_t offset) {
    const uint64_t ne0 = src->ne[0];
    const uint64_t ne1 = src->ne[1];
    const enum ggml_type type = src->type;
    const size_t ts = ggml_type_size(type);
    ...
    size_t size = row_size * ne1;
    const char * start_ptr = x + offset;
    cl_mem_flags mem_flags = CL_MEM_USE_HOST_PTR | CL_MEM_READ_WRITE;
    const cl_mem new_dst = clCreateBuffer(_global_context, mem_flags, size, (void *)start_ptr, &err);
    if (err != CL_SUCCESS) {
        fprintf(stderr, "ggml_opencl: clCreateBuffer error %d at %s:%d\n", err, __FILE__, __LINE__);
        // get the error string
        size_t len;
...
static cl_mem ggml_cl_h2d_tensor_2d_with_unified_mem(const void * ptr, size_t offset, size_t size)
{
    cl_int err = CL_SUCCESS;
    cl_mem_flags mem_flags = CL_MEM_USE_HOST_PTR | CL_MEM_READ_WRITE;
    const cl_mem new_dst = clCreateBuffer(_global_context, mem_flags, size, (void *)ptr, &err);
    if (err != CL_SUCCESS) {
        fprintf(stderr, "ggml_opencl: clCreateBuffer error %d at %s:%d\n", err, __FILE__, __LINE__);
        // get the error string
        size_t len;
...
static void ggml_cl_mul_f32(const ggml_tensor * src0, const ggml_tensor * src1, ggml_tensor * dst) {
    ...
    for (int64_t i03 = 0; i03 < ne03; i03++) {
        for (int64_t i02 = 0; i02 < ne02; i02++) {
            ...
            // copy src0 to device
            CL_CHECK(ggml_cl_h2d_tensor_2d(_global_queue, d_X, 0, src0, i03, i02, &ev));
            ...
            cl_kernel mul_f32_cl = get_kernel("mul_f32");
            ...
            CL_CHECK(clEnqueueNDRangeKernel(_global_queue, mul_f32_cl, 1, NULL, &global, NULL, 1, &ev, NULL));
            ...
            CL_CHECK(clEnqueueReadBuffer(_global_queue, d_D, true, 0, sizeof(float) * ne00*ne01, d, 0, NULL, NULL));
        }
    }
        ...
        for (int64_t i13 = i03 * r3, e13 = i13 + r3; i13 < e13; i13++) {
            for (int64_t i02 = 0; i02 < ne02; i02++) {
                if (src0->backend == GGML_BACKEND_GPU) {
                    ...
                    // copy src0 to device
                    CL_CHECK(ggml_cl_h2d_tensor_2d(_global_queue, d_X, 0, src0, i03, i02, NULL));
                }
                ...
                for (int64_t i12 = i02 * r2, e12 = i12 + r2; i12 < e12; i12++) {
                    ...
                    CL_CHECK(clEnqueueReadBuffer(_global_queue, d_D, true, 0, sizeof(float) * d_ne, d, 1, &ev_sgemm, NULL));
                }
            }
        }
    }
        ...
        for (int64_t i13 = i03 * r3, e13 = i13 + r3; i13 < e13; i13++) {
            for (int64_t i02 = 0; i02 < ne02; i02++) {
                ...
                for (int64_t i12 = i02 * r2, e12 = i12 + r2; i12 < e12; i12++) {
                    ...
                    // copy src1 to device
                    CL_CHECK(clEnqueueWriteBuffer(_global_queue, d_Y, false, 0, sizeof(ggml_fp16_t) * y_ne, tmp, 0, NULL, NULL));
                    ...
                    CL_CHECK(clEnqueueReadBuffer(_global_queue, d_D, true, 0, sizeof(ggml_fp16_t) * d_ne, tmp, 1, &ev_sgemm, NULL));
...
static void ggml_cl_mul_mat_q_f32(const ggml_tensor * src0, const ggml_tensor * src1, ggml_tensor * dst) {
    ...
    cl_mem d_X = ggml_cl_pool_malloc(sizeof(float) * x_ne);
    cl_mem d_Y;
    cl_mem d_D;
    ...
    if (mul_mat_vec) { // specialized dequantize_mul_mat_vec kernel
        ...
        // get raw ptr from buffer d_X

        events.emplace_back();
        // Assuming d_X, d_Y, and d_D are allocated in unified memory
        float* d_X_cpu = static_cast<float*>(get_buffer_mem_ptr());;  // Pointer to d_X in CPU memory
        float* d_Y_cpu = static_cast<float*>(src1->data);  // Pointer to d_Y in CPU memory
...
void ggml_cl_transform_tensor(void * data, ggml_tensor * tensor) {
    ...
    for (int64_t i3 = 0; i3 < ne3; i3++) {
        for (int64_t i2 = 0; i2 < ne2; i2++) {
            CL_CHECK(ggml_cl_h2d_tensor_2d(_global_queue, dst, offset, tensor, i3, i2, NULL));
            offset += s_sz;
        }
    }
