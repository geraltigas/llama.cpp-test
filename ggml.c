...
enum ggml_type ggml_ftype_to_ggml_type(enum ggml_ftype ftype) {
    ...
    switch (ftype) {
        case GGML_FTYPE_ALL_F32: wtype = GGML_TYPE_F32;
            break;
        case GGML_FTYPE_MOSTLY_F16: wtype = GGML_TYPE_F16;
            break;
        case GGML_FTYPE_MOSTLY_Q4_0: wtype = GGML_TYPE_Q4_0;
            break;
        case GGML_FTYPE_MOSTLY_Q4_1: wtype = GGML_TYPE_Q4_1;
            break;
        case GGML_FTYPE_MOSTLY_Q5_0: wtype = GGML_TYPE_Q5_0;
            break;
        case GGML_FTYPE_MOSTLY_Q5_1: wtype = GGML_TYPE_Q5_1;
            break;
        case GGML_FTYPE_MOSTLY_Q8_0: wtype = GGML_TYPE_Q8_0;
            break;
        case GGML_FTYPE_MOSTLY_Q2_K: wtype = GGML_TYPE_Q2_K;
            break;
        case GGML_FTYPE_MOSTLY_Q3_K: wtype = GGML_TYPE_Q3_K;
            break;
        case GGML_FTYPE_MOSTLY_Q4_K: wtype = GGML_TYPE_Q4_K;
            break;
        case GGML_FTYPE_MOSTLY_Q5_K: wtype = GGML_TYPE_Q5_K;
            break;
        case GGML_FTYPE_MOSTLY_Q6_K: wtype = GGML_TYPE_Q6_K;
            break;
        case GGML_FTYPE_UNKNOWN: wtype = GGML_TYPE_COUNT;
            break;
        case GGML_FTYPE_MOSTLY_Q4_1_SOME_F16: wtype = GGML_TYPE_COUNT;
            break;
    }
...
int32_t ggml_get_i32_nd(const struct ggml_tensor* tensor, int i0, int i1, int i2, int i3) {
    void* data = (char *)tensor->data + i0 * tensor->nb[0] + i1 * tensor->nb[1] + i2 * tensor->nb[2] + i3 * tensor->nb[
                     3];
    switch (tensor->type) {
        case GGML_TYPE_I8:
...
static struct ggml_tensor* ggml_rope_impl(
    ...
    int32_t params[13] = {
        /*n_past*/ 0, n_dims, mode, n_ctx, n_orig_ctx
    };
            ...
            if (dst->type == GGML_TYPE_F16) {
                ...
                for (int i03 = 0; i03 < ne03; i03++) {
                    for (int i02 = 0; i02 < ne02; i02++) {
                        id += ne00 * ir0;
                        for (int i01 = ir0; i01 < ir1; i01++) {
                            const ggml_fp16_t* src0_ptr = (ggml_fp16_t *)(
                                (char *)src0->data + i01 * nb01 + i02 * nb02 + i03 * nb03);
...
static void ggml_compute_forward_acc_f32(
    ...
    GGML_ASSERT(
        offset + (ne10 == 0 ? 0 : ne10-1)*nb0 + (ne11 == 0 ? 0 : ne11-1)*nb1 + (ne12 == 0 ? 0 : ne12-1)*nb2 + (ne13 == 0
            ? 0 : ne13-1)*nb3 < ggml_nbytes(dst));
    GGML_ASSERT(
        offset + (ne10 == 0 ? 0 : ne10-1)*nb00 + (ne11 == 0 ? 0 : ne11-1)*nb01 + (ne12 == 0 ? 0 : ne12-1)*nb02 + (ne13
            == 0 ? 0 : ne13-1)*nb03 < ggml_nbytes(src0));
                        ...
                        for (int k1 = 0; k1 < ne01; k1++) {
                            for (int i0 = 0; i0 < nr0; i0++) {
                                ggml_vec_cpy_f32(ne00,
                                                 (float *)((char *)dst->data + (i3 * ne03 + k3) * nb3 + (i2 * ne02 + k2)
                                                           * nb2 + (i1 * ne01 + k1) * nb1 + (i0 * ne00) * nb0),
                                                 (float *)((char *)src0->data + (k3) * nb03 + (k2) * nb02 + (k1) *
                                                           nb01));
                            }
                        }
                    }
            ...
            for (int i2 = 0; i2 < nr2; i2++) {
                for (int k2 = 0; k2 < ne02; k2++) {
                    for (int i1 = 0; i1 < nr1; i1++) {
                        for (int k1 = 0; k1 < ne01; k1++) {
                            for (int i0 = 0; i0 < nr0; i0++) {
                                ggml_fp16_t* y = (ggml_fp16_t *)(
                                    (char *)dst->data + (i3 * ne03 + k3) * nb3 + (i2 * ne02 + k2) * nb2 + (
                                        i1 * ne01 + k1) * nb1 + (i0 * ne00) * nb0);
                                ggml_fp16_t* x = (ggml_fp16_t *)(
                                    (char *)src0->data + (k3) * nb03 + (k2) * nb02 + (k1) * nb01);
                        ...
                        for (int k1 = 0; k1 < ne1; k1++) {
                            for (int i0 = 0; i0 < nr0; i0++) {
                                ggml_vec_acc_f32(ne0,
                                                 ...
                                                 (float *)((char *)src0->data + (i3 * ne3 + k3) * nb03 + (i2 * ne2 + k2)
                                                           * nb02 + (i1 * ne1 + k1) * nb01 + (i0 * ne0) * nb00));
                            }
                        }
                    }
                }
        ...
        for (int i2 = ith; i2 < ne2; i2 += nth) {
            if (i2 < ne02) {
                ...
                for (int i1 = 0; i1 < ne1; i1++) {
                    for (int i0 = 0; i0 < ne0; i0++) {
                        const float* x = (float *)(
                            (char *)src1->data + i0 * nb10 + i1 * nb11 + (i2 - ne02) * nb12 + i3 * nb13);
...
static void ggml_compute_forward_mul_mat_id(
    ...
    char* wdata_src1_end = (src1->type == vec_dot_type)
                               ? (char *)params->wdata
                               : (char *)params->wdata + GGML_PAD(ggml_row_size(vec_dot_type, ggml_nelements(src1)),
                                                                  sizeof(int64_t));
            ...
            for (int64_t i1 = 0; i1 < ne1; i1++) {
                ...
                if (is_glm) {
                    ...
                    for (int64_t i0 = 0; i0 < ne0 / 4; i0++) {
                        ...
                        const float* const src = (float *)(
                            (char *)src0->data + i3 * nb03 + i2 * nb02 + i1 * nb01 + i0 * nb00);
                ...
                if (is_glm) {
                    ...
                    for (int64_t i0 = 0; i0 < ne0 / 4; i0++) {
                        ...
                        const ggml_fp16_t* const src = (ggml_fp16_t *)(
                            (char *)src0->data + i3 * nb03 + i2 * nb02 + i1 * nb01 + i0 * nb00);
                        ggml_fp16_t* dst_data = (ggml_fp16_t *)(
                            (char *)dst->data + i3 * nb3 + i2 * nb2 + i1 * nb1 + i0 * nb0);
...
static void ggml_compute_forward_pool_1d_sk_p0(
    ...
    while (cdata < data_end) {
        ...
        for (int64_t i = 0; i < rs; ++i) {
            switch (op) {
                case GGML_OP_POOL_AVG: drow[i] = 0;
                    break;
                case GGML_OP_POOL_MAX: drow[i] = -FLT_MAX;
                    break;
            ...
            for (int ki = 0; ki < k; ++ki) {
                switch (op) {
                    case GGML_OP_POOL_AVG: drow[i] += srow[j];
                        break;
                    case GGML_OP_POOL_MAX: if (srow[j] > drow[i]) drow[i] = srow[j];
                        break;
                    case GGML_OP_POOL_COUNT: GGML_ASSERT(false);
                        break;
                }
            ...
            switch (op) {
                case GGML_OP_POOL_AVG: drow[i] /= k;
                    break;
    ...
    while (cdata < data_end) {
        for (int oy = 0; oy < py; ++oy) {
            ...
            for (int ox = 0; ox < px; ++ox) {
                ...
                switch (op) {
                    case GGML_OP_POOL_AVG: *out = 0;
                        break;
                    case GGML_OP_POOL_MAX: *out = -FLT_MAX;
                        break;
                    case GGML_OP_POOL_COUNT: GGML_ASSERT(false);
                        break;
                }
                ...
                for (int ky = 0; ky < k1; ++ky) {
                    ...
                    for (int kx = 0; kx < k0; ++kx) {
                        ...
                        switch (op) {
                            case GGML_OP_POOL_AVG: *out += srow[j];
                                break;
                            case GGML_OP_POOL_MAX: if (srow[j] > *out) *out = srow[j];
                                break;
                            case GGML_OP_POOL_COUNT: GGML_ASSERT(false);
                                break;
                        }
        ...
        for (int64_t i1 = ith; i1 < ne1; i1 += nth) {
            for (int64_t i0 = 0; i0 < ne0; ++i0) {
                for (int64_t i3 = 0; i3 < ne3; ++i3) {
                    ...
                    const float* src_ptr = (const float *)(
                        (char *)src0->data + i3 * nb03 + i2 * nb02 + i1 * nb01 + i0 * nb00);
    ...
    for (int ir = ir0; ir < ir1; ++ir) {
        ...
        for (int irep = 0; irep < nrep; ++irep) {
            ...
            for (int iq1 = 0; iq1 < neq1; ++iq1) {
                ...
                float* S = (float *)params->wdata + ith * 2 * (mxDM + CACHE_LINE_SIZE_F32) + 0 * (
                               mxDM + CACHE_LINE_SIZE_F32);
                float* SM = (float *)params->wdata + ith * 2 * (mxDM + CACHE_LINE_SIZE_F32) + 1 * (
                                mxDM + CACHE_LINE_SIZE_F32);
...
static void ggml_compute_forward(struct ggml_compute_params* params, struct ggml_tensor* tensor) {
    ...
    switch (tensor->op) {
        case GGML_OP_DUP: {
            start_named_timer("DUP");
            ggml_compute_forward_dup(params, tensor->src[0], tensor);
            stop_named_timer_and_record("DUP");
        }
        break;
        case GGML_OP_ADD: {
            start_named_timer("ADD");
            ggml_compute_forward_add(params, tensor->src[0], tensor->src[1], tensor);
            stop_named_timer_and_record("ADD");
        }
        break;
        case GGML_OP_ADD1: {
            start_named_timer("ADD1");
            ggml_compute_forward_add1(params, tensor->src[0], tensor->src[1], tensor);
            start_named_timer("ADD1");
        }
        break;
        case GGML_OP_ACC: {
            start_named_timer("ACC");
            ggml_compute_forward_acc(params, tensor->src[0], tensor->src[1], tensor);
            stop_named_timer_and_record("ACC");
        }
        break;
        case GGML_OP_SUB: {
            start_named_timer("SUB");
            ggml_compute_forward_sub(params, tensor->src[0], tensor->src[1], tensor);
            stop_named_timer_and_record("SUB");
        }
        break;
        case GGML_OP_MUL: {
            start_named_timer("MUL");
            ggml_compute_forward_mul(params, tensor->src[0], tensor->src[1], tensor);
            stop_named_timer_and_record("MUL");
        }
        break;
        case GGML_OP_DIV: {
            start_named_timer("DIV");
            ggml_compute_forward_div(params, tensor->src[0], tensor->src[1], tensor);
            stop_named_timer_and_record("DIV");
        }
        break;
        case GGML_OP_SQR: {
            start_named_timer("SQR");
            ggml_compute_forward_sqr(params, tensor->src[0], tensor);
            stop_named_timer_and_record("SQR");
        }
        break;
        case GGML_OP_SQRT: {
            start_named_timer("SQRT");
            ggml_compute_forward_sqrt(params, tensor->src[0], tensor);
            stop_named_timer_and_record("SQRT");
        }
        break;
        case GGML_OP_LOG: {
            start_named_timer("LOG");
            ggml_compute_forward_log(params, tensor->src[0], tensor);
            stop_named_timer_and_record("LOG");
        }
        break;
        case GGML_OP_SUM: {
            start_named_timer("SUM");
            ggml_compute_forward_sum(params, tensor->src[0], tensor);
            stop_named_timer_and_record("SUM");
        }
        break;
        case GGML_OP_SUM_ROWS: {
            start_named_timer("SUM_ROWS");
            ggml_compute_forward_sum_rows(params, tensor->src[0], tensor);
            stop_named_timer_and_record("SUM_ROWS");
        }
        break;
        case GGML_OP_MEAN: {
            start_named_timer("MEANS");
            ggml_compute_forward_mean(params, tensor->src[0], tensor);
            stop_named_timer_and_record("MEANS");
        }
        break;
        case GGML_OP_ARGMAX: {
            start_named_timer("ARGMAX");
            ggml_compute_forward_argmax(params, tensor->src[0], tensor);
            stop_named_timer_and_record("ARGMAX");
        }
        break;
        case GGML_OP_REPEAT: {
            start_named_timer("REPEAT");
            ggml_compute_forward_repeat(params, tensor->src[0], tensor);
            stop_named_timer_and_record("REPEAT");
        }
        break;
        case GGML_OP_REPEAT_BACK: {
            start_named_timer("REPEAT_BACK");
            ggml_compute_forward_repeat_back(params, tensor->src[0], tensor);
            stop_named_timer_and_record("REPEAT_BACK");
        }
        break;
        case GGML_OP_CONCAT: {
            start_named_timer("CONCAT");
            ggml_compute_forward_concat(params, tensor->src[0], tensor->src[1], tensor);
            stop_named_timer_and_record("CONCAT");
        }
        break;
        case GGML_OP_NORM: {
            start_named_timer("NORM");
            ggml_compute_forward_norm(params, tensor->src[0], tensor);
            stop_named_timer_and_record("NORM");
        }
        break;
        case GGML_OP_RMS_NORM: {
            start_named_timer("RMS_NORM");
            ggml_compute_forward_rms_norm(params, tensor->src[0], tensor);
            stop_named_timer_and_record("RMS_NORM");
        }
        break;
        case GGML_OP_RMS_NORM_BACK: {
            start_named_timer("RMS_NORM_BACK");
            ggml_compute_forward_rms_norm_back(params, tensor->src[0], tensor->src[1], tensor);
            stop_named_timer_and_record("RMS_NORM_BACK");
        }
        break;
        case GGML_OP_GROUP_NORM: {
            start_named_timer("GROUP_NORM");
            ggml_compute_forward_group_norm(params, tensor->src[0], tensor);
            stop_named_timer_and_record("GROUP_NORM");
        }
        break;
        case GGML_OP_MUL_MAT: {
            start_named_timer("MUL_MAT");
            ggml_compute_forward_mul_mat(params, tensor->src[0], tensor->src[1], tensor);
            stop_named_timer_and_record("MUL_MAT");
        }
        break;
        case GGML_OP_MUL_MAT_ID: {
            start_named_timer("MUL_MAT_ID");
            ggml_compute_forward_mul_mat_id(params, tensor->src[0], tensor->src[1], tensor);
            stop_named_timer_and_record("MUL_MAT_ID");
        }
        break;
        case GGML_OP_OUT_PROD: {
            start_named_timer("OUT_PROD");
            ggml_compute_forward_out_prod(params, tensor->src[0], tensor->src[1], tensor);
            stop_named_timer_and_record("OUT_PROD");
        }
        break;
        case GGML_OP_SCALE: {
        ...
        case GGML_OP_SET: {
            ...
            stop_named_timer_and_record("SET");
        }
        break;
        case GGML_OP_CPY: {
            start_named_timer("CPY");
            ggml_compute_forward_cpy(params, tensor->src[0], tensor);
            stop_named_timer_and_record("CPY");
        }
        break;
        case GGML_OP_CONT: {
            start_named_timer("CONT");
            ggml_compute_forward_cont(params, tensor->src[0], tensor);
            stop_named_timer_and_record("CONT");
        }
        break;
        case GGML_OP_RESHAPE: {
            start_named_timer("RESHAPE");
            ggml_compute_forward_reshape(params, tensor->src[0], tensor);
            stop_named_timer_and_record("RESHAPE");
        }
        break;
        case GGML_OP_VIEW: {
            start_named_timer("VIEW");
            ggml_compute_forward_view(params, tensor->src[0]);
            stop_named_timer_and_record("VIEW");
        }
        break;
        case GGML_OP_PERMUTE: {
            start_named_timer("PERMUTE");
            ggml_compute_forward_permute(params, tensor->src[0]);
            stop_named_timer_and_record("PERMUTE");
        }
        break;
        case GGML_OP_TRANSPOSE: {
            start_named_timer("TRANSPOSE");
            ggml_compute_forward_transpose(params, tensor->src[0]);
            stop_named_timer_and_record("TRANSPOSE");
        }
        break;
        case GGML_OP_GET_ROWS: {
            start_named_timer("GET_ROWS");
            ggml_compute_forward_get_rows(params, tensor->src[0], tensor->src[1], tensor);
            stop_named_timer_and_record("GET_ROWS");
        }
        break;
        case GGML_OP_GET_ROWS_BACK: {
            start_named_timer("GET_ROWS_BACK");
            ggml_compute_forward_get_rows_back(params, tensor->src[0], tensor->src[1], tensor);
            stop_named_timer_and_record("GET_ROWS_BACK");
        }
        break;
        case GGML_OP_DIAG: {
            start_named_timer("DIAG");
            ggml_compute_forward_diag(params, tensor->src[0], tensor);
            stop_named_timer_and_record("DIAG");
        }
        break;
        case GGML_OP_DIAG_MASK_INF: {
            start_named_timer("DIAG_MASK_INF");
            ggml_compute_forward_diag_mask_inf(params, tensor->src[0], tensor);
            stop_named_timer_and_record("DIAG_MASK_INF");
        }
        break;
        case GGML_OP_DIAG_MASK_ZERO: {
            start_named_timer("DIAG_MASK_ZERO");
            ggml_compute_forward_diag_mask_zero(params, tensor->src[0], tensor);
            stop_named_timer_and_record("DIAG_MASK_ZERO");
        }
        break;
        case GGML_OP_SOFT_MAX: {
            start_named_timer("SOFT_MAX");
            ggml_compute_forward_soft_max(params, tensor->src[0], tensor->src[1], tensor);
            stop_named_timer_and_record("SOFT_MAX");
        }
        break;
        case GGML_OP_SOFT_MAX_BACK: {
            start_named_timer("SOFT_MAX_BACK");
            ggml_compute_forward_soft_max_back(params, tensor->src[0], tensor->src[1], tensor);
            stop_named_timer_and_record("SOFT_MAX_BACK");
        }
        break;
        case GGML_OP_ROPE: {
            start_named_timer("ROPE");
            ggml_compute_forward_rope(params, tensor->src[0], tensor->src[1], tensor);
            stop_named_timer_and_record("ROPE");
        }
        break;
        case GGML_OP_ROPE_BACK: {
            start_named_timer("ROPE_BACK");
            ggml_compute_forward_rope_back(params, tensor->src[0], tensor->src[1], tensor);
            stop_named_timer_and_record("ROPE_BACK");
        }
        break;
        case GGML_OP_ALIBI: {
            start_named_timer("ALIBI");
            ggml_compute_forward_alibi(params, tensor->src[0], tensor);
            stop_named_timer_and_record("ALIBI");
        }
        break;
        case GGML_OP_CLAMP: {
            start_named_timer("CLAMP");
            ggml_compute_forward_clamp(params, tensor->src[0], tensor);
            stop_named_timer_and_record("CLAMP");
        }
        break;
        case GGML_OP_CONV_TRANSPOSE_1D: {
            start_named_timer("CONV_TRANSPOSE_1D");
            ggml_compute_forward_conv_transpose_1d(params, tensor->src[0], tensor->src[1], tensor);
            stop_named_timer_and_record("CONV_TRANSPOSE_1D");
        }
        break;
        case GGML_OP_IM2COL: {
            start_named_timer("IM2COL");
            ggml_compute_forward_im2col(params, tensor->src[0], tensor->src[1], tensor);
            stop_named_timer_and_record("IM2COL");
        }
        break;
        case GGML_OP_CONV_TRANSPOSE_2D: {
            start_named_timer("CONV_TRANSPOSE_2D");
            ggml_compute_forward_conv_transpose_2d(params, tensor->src[0], tensor->src[1], tensor);
            stop_named_timer_and_record("CONV_TRANSPOSE_2D");
        }
        break;
        case GGML_OP_POOL_1D: {
            start_named_timer("POOL_1D");
            ggml_compute_forward_pool_1d(params, tensor->src[0], tensor);
            stop_named_timer_and_record("POOL_1D");
        }
        break;
        case GGML_OP_POOL_2D: {
            start_named_timer("POOL_2D");
            ggml_compute_forward_pool_2d(params, tensor->src[0], tensor);
            stop_named_timer_and_record("POOL_2D");
        }
        break;
        case GGML_OP_UPSCALE: {
            start_named_timer("UPSCALE");
            ggml_compute_forward_upscale(params, tensor->src[0], tensor);
            stop_named_timer_and_record("UPSCALE");
        }
        break;
        case GGML_OP_PAD: {
            start_named_timer("PAD");
            ggml_compute_forward_pad(params, tensor->src[0], tensor);
            stop_named_timer_and_record("PAD");
        }
        break;
        case GGML_OP_ARGSORT: {
            start_named_timer("ARGSORT");
            ggml_compute_forward_argsort(params, tensor->src[0], tensor);
            stop_named_timer_and_record("ARGSORT");
        }
        break;
        case GGML_OP_LEAKY_RELU: {
            start_named_timer("LEAKY_RELU");
            ggml_compute_forward_leaky_relu(params, tensor->src[0], tensor);
            stop_named_timer_and_record("LEAKY_RELU");
        }
        break;
        case GGML_OP_FLASH_ATTN: {
            start_named_timer("FLASH_ATTN");
            const int32_t t = ggml_get_op_params_i32(tensor, 0);
            GGML_ASSERT(t == 0 || t == 1);
            const bool masked = t != 0;
            ggml_compute_forward_flash_attn(params, tensor->src[0], tensor->src[1], tensor->src[2], masked, tensor);
            stop_named_timer_and_record("FLASH_ATTN");
        }
        break;
        case GGML_OP_FLASH_FF: {
            start_named_timer("FLASH_FF");
            ggml_compute_forward_flash_ff(params, tensor->src[0], tensor->src[1], tensor->src[2], tensor->src[3],
                                          tensor->src[4], tensor);
            stop_named_timer_and_record("FLASH_FF");
        }
        break;
        case GGML_OP_FLASH_ATTN_BACK: {
            start_named_timer("FLASH_ATTN_BACK");
            int32_t t = ggml_get_op_params_i32(tensor, 0);
            GGML_ASSERT(t == 0 || t == 1);
            bool masked = t != 0;
            ggml_compute_forward_flash_attn_back(params, tensor->src[0], tensor->src[1], tensor->src[2], tensor->src[3],
                                                 masked, tensor);
            stop_named_timer_and_record("FLASH_ATTN_BACK");
        }
        break;
        case GGML_OP_WIN_PART: {
            start_named_timer("WIN_PART");
            ggml_compute_forward_win_part(params, tensor->src[0], tensor);
            stop_named_timer_and_record("WIN_PART");
        }
        break;
        case GGML_OP_WIN_UNPART: {
            start_named_timer("WIN_UNPART");
            ggml_compute_forward_win_unpart(params, tensor->src[0], tensor);
            stop_named_timer_and_record("WIN_UNPART");
        }
        break;
        case GGML_OP_UNARY: {
            start_named_timer("UNARY");
            ggml_compute_forward_unary(params, tensor->src[0], tensor);
            stop_named_timer_and_record("UNARY");
        }
        break;
        case GGML_OP_GET_REL_POS: {
            start_named_timer("GET_REL_POS");
            ggml_compute_forward_get_rel_pos(params, tensor->src[0], tensor);
            stop_named_timer_and_record("GET_REL_POS");
        }
        break;
        case GGML_OP_ADD_REL_POS: {
            start_named_timer("ADD_REL_POS");
            ggml_compute_forward_add_rel_pos(params, tensor->src[0], tensor->src[1], tensor->src[2], tensor);
            stop_named_timer_and_record("ADD_REL_POS");
        }
        break;
        case GGML_OP_MAP_UNARY: {
            ggml_unary_op_f32_t fun;
            memcpy(&fun, tensor->op_params, sizeof(fun));
            start_named_timer("MAP_UNARY");
            ggml_compute_forward_map_unary(params, tensor->src[0], tensor, fun);
            stop_named_timer_and_record("MAP_UNARY");
        }
        break;
        case GGML_OP_MAP_BINARY: {
            ggml_binary_op_f32_t fun;
            memcpy(&fun, tensor->op_params, sizeof(fun));
            start_named_timer("MAP_BINARY");
            ggml_compute_forward_map_binary(params, tensor->src[0], tensor->src[1], tensor, fun);
            stop_named_timer_and_record("MAP_BINARY");
        }
        break;
        case GGML_OP_MAP_CUSTOM1_F32: {
            ggml_custom1_op_f32_t fun;
            memcpy(&fun, tensor->op_params, sizeof(fun));
            start_named_timer("MAP_CUSTOM1_F32");
            ggml_compute_forward_map_custom1_f32(params, tensor->src[0], tensor, fun);
            stop_named_timer_and_record("MAP_CUSTOM1_F32");
        }
        break;
        case GGML_OP_MAP_CUSTOM2_F32: {
            ggml_custom2_op_f32_t fun;
            memcpy(&fun, tensor->op_params, sizeof(fun));
            start_named_timer("MAP_CUSTOM2_F32");
            ggml_compute_forward_map_custom2_f32(params, tensor->src[0], tensor->src[1], tensor, fun);
            stop_named_timer_and_record("MAP_CUSTOM2_F32");
        }
        break;
        case GGML_OP_MAP_CUSTOM3_F32: {
            ggml_custom3_op_f32_t fun;
            memcpy(&fun, tensor->op_params, sizeof(fun));
            start_named_timer("MAP_CUSTOM3_F32");
            ggml_compute_forward_map_custom3_f32(params, tensor->src[0], tensor->src[1], tensor->src[2], tensor, fun);
            stop_named_timer_and_record("MAP_CUSTOM3_F32");
        }
        break;
        case GGML_OP_MAP_CUSTOM1: {
            start_named_timer("MAP_CUSTOM1");
            ggml_compute_forward_map_custom1(params, tensor->src[0], tensor);
            stop_named_timer_and_record("MAP_CUSTOM1");
        }
        break;
        case GGML_OP_MAP_CUSTOM2: {
            start_named_timer("MAP_CUSTOM2");
            ggml_compute_forward_map_custom2(params, tensor->src[0], tensor->src[1], tensor);
            stop_named_timer_and_record("MAP_CUSTOM2");
        }
        break;
        case GGML_OP_MAP_CUSTOM3: {
            start_named_timer("MAP_CUSTOM3");
            ggml_compute_forward_map_custom3(params, tensor->src[0], tensor->src[1], tensor->src[2], tensor);
            stop_named_timer_and_record("MAP_CUSTOM3");
        }
        break;
        case GGML_OP_CROSS_ENTROPY_LOSS: {
            start_named_timer("CROSS_ENTROPY_LOSS");
            ggml_compute_forward_cross_entropy_loss(params, tensor->src[0], tensor->src[1], tensor);
            stop_named_timer_and_record("CROSS_ENTROPY_LOSS");
        }
        break;
        case GGML_OP_CROSS_ENTROPY_LOSS_BACK: {
            start_named_timer("CROSS_ENTROPY_LOSS_BACK");
            ggml_compute_forward_cross_entropy_loss_back(params, tensor->src[0], tensor->src[1], tensor->src[2],
                                                         tensor);
            stop_named_timer_and_record("CROSS_ENTROPY_LOSS_BACK");
        }
        break;
        case GGML_OP_NONE: {
            // nop
...
static struct ggml_tensor* ggml_acc_or_set(struct ggml_context* ctx, struct ggml_tensor* a, struct ggml_tensor* b,
        ...
        case GGML_OP_ADD1: {
            ...
            if (src1->grad) {
                src1->grad = ggml_add_or_set(ctx,
                                             ...
                                             // TODO: should probably be sum instead of mean
                                             zero_table);
            }
        }
        break;
...
static void ggml_graph_export_node(const struct ggml_tensor* tensor, const char* arg, FILE* fout) {
    ...
    fprintf(
        fout, "%-6s %-6s %-12s %8d %" PRId64 " %" PRId64 " %" PRId64 " %" PRId64 " %16zu %16zu %16zu %16zu %16p %32s\n",
        arg,
        ggml_type_name(tensor->type),
        ggml_op_name(tensor->op),
        ggml_n_dims(tensor),
    ...
    if (opt->ctx == NULL) {
        ...
        if (opt->params.type == GGML_OPT_ADAM) {
            ...
            if (opt->params.past > 0) {
                ctx_opt_params.mem_size += GGML_MEM_ALIGN + ggml_tensor_overhead() + ggml_type_size(GGML_TYPE_F32) * opt
                        ->params.past;
            }
        }
        else if (opt->params.type == GGML_OPT_LBFGS) {
            ctx_opt_params.mem_size =
                    GGML_MEM_ALIGN * 9 + ggml_tensor_overhead() * 9 + ggml_type_size(GGML_TYPE_F32) * (
        ...
        for (uint64_t i = 0; i < ctx->header.n_kv; ++i) {
            ...
            switch (kv->type) {
                case GGUF_TYPE_UINT8:
                    ...
                    break;
                case GGUF_TYPE_INT8: ok = ok && gguf_fread_el(file, &kv->value.int8, sizeof(kv->value.int8), &offset);
                    break;
                case GGUF_TYPE_UINT16: ok = ok && gguf_fread_el(file, &kv->value.uint16, sizeof(kv->value.uint16),
                                                                &offset);
                    break;
                case GGUF_TYPE_INT16:
                    ok = ok && gguf_fread_el(file, &kv->value.int16, sizeof(kv->value.int16), &offset);
                    break;
                case GGUF_TYPE_UINT32: ok = ok && gguf_fread_el(file, &kv->value.uint32, sizeof(kv->value.uint32),
                                                                &offset);
                    break;
                case GGUF_TYPE_INT32:
                    ok = ok && gguf_fread_el(file, &kv->value.int32, sizeof(kv->value.int32), &offset);
                    break;
                case GGUF_TYPE_FLOAT32: ok = ok && gguf_fread_el(file, &kv->value.float32, sizeof(kv->value.float32),
                                                                 &offset);
                    break;
                case GGUF_TYPE_UINT64: ok = ok && gguf_fread_el(file, &kv->value.uint64, sizeof(kv->value.uint64),
                                                                &offset);
                    break;
                case GGUF_TYPE_INT64:
                    ok = ok && gguf_fread_el(file, &kv->value.int64, sizeof(kv->value.int64), &offset);
                    break;
                case GGUF_TYPE_FLOAT64: ok = ok && gguf_fread_el(file, &kv->value.float64, sizeof(kv->value.float64),
                                                                 &offset);
                    break;
                case GGUF_TYPE_BOOL: ok = ok && gguf_fread_el(file, &kv->value.bool_, sizeof(kv->value.bool_), &offset);
                    break;
                case GGUF_TYPE_STRING: ok = ok && gguf_fread_str(file, &kv->value.str, &offset);
                    break;
    ...
    // compute the total size of the data section, taking into account the alignment
    {
        ...
        for (uint64_t i = 0; i < ctx->header.n_tensors; ++i) {
            ...
            if (ne % ggml_blck_size(info->type) != 0) {
                fprintf(
                    stderr, "%s: tensor '%s' number of elements (%" PRId64 ") is not a multiple of block size (%d)\n",
...
void gguf_set_kv(struct gguf_context* ctx, struct gguf_context* src) {
    for (uint32_t i = 0; i < src->header.n_kv; i++) {
        switch (src->kv[i].type) {
            case GGUF_TYPE_UINT8: gguf_set_val_u8(ctx, src->kv[i].key.data, src->kv[i].value.uint8);
                break;
            case GGUF_TYPE_INT8: gguf_set_val_i8(ctx, src->kv[i].key.data, src->kv[i].value.int8);
                break;
            case GGUF_TYPE_UINT16: gguf_set_val_u16(ctx, src->kv[i].key.data, src->kv[i].value.uint16);
                break;
            case GGUF_TYPE_INT16: gguf_set_val_i16(ctx, src->kv[i].key.data, src->kv[i].value.int16);
                break;
            case GGUF_TYPE_UINT32: gguf_set_val_u32(ctx, src->kv[i].key.data, src->kv[i].value.uint32);
                break;
            case GGUF_TYPE_INT32: gguf_set_val_i32(ctx, src->kv[i].key.data, src->kv[i].value.int32);
                break;
            case GGUF_TYPE_FLOAT32: gguf_set_val_f32(ctx, src->kv[i].key.data, src->kv[i].value.float32);
                break;
            case GGUF_TYPE_UINT64: gguf_set_val_u64(ctx, src->kv[i].key.data, src->kv[i].value.uint64);
                break;
            case GGUF_TYPE_INT64: gguf_set_val_i64(ctx, src->kv[i].key.data, src->kv[i].value.int64);
                break;
            case GGUF_TYPE_FLOAT64: gguf_set_val_f64(ctx, src->kv[i].key.data, src->kv[i].value.float64);
                break;
            case GGUF_TYPE_BOOL: gguf_set_val_bool(ctx, src->kv[i].key.data, src->kv[i].value.bool_);
                break;
            case GGUF_TYPE_STRING: gguf_set_val_str(ctx, src->kv[i].key.data, src->kv[i].value.str.data);
                break;
            case GGUF_TYPE_ARRAY: {
...
static void gguf_write_to_buf(const struct gguf_context* ctx, struct gguf_buf* buf, bool only_meta) {
    ...
    for (uint32_t i = 0; i < ctx->header.n_kv; ++i) {
        ...
        switch (kv->type) {
            case GGUF_TYPE_UINT8: gguf_bwrite_el(buf, &kv->value.uint8, sizeof(kv->value.uint8));
                break;
            case GGUF_TYPE_INT8: gguf_bwrite_el(buf, &kv->value.int8, sizeof(kv->value.int8));
                break;
            case GGUF_TYPE_UINT16: gguf_bwrite_el(buf, &kv->value.uint16, sizeof(kv->value.uint16));
                break;
            case GGUF_TYPE_INT16: gguf_bwrite_el(buf, &kv->value.int16, sizeof(kv->value.int16));
                break;
            case GGUF_TYPE_UINT32: gguf_bwrite_el(buf, &kv->value.uint32, sizeof(kv->value.uint32));
                break;
            case GGUF_TYPE_INT32: gguf_bwrite_el(buf, &kv->value.int32, sizeof(kv->value.int32));
                break;
            case GGUF_TYPE_FLOAT32: gguf_bwrite_el(buf, &kv->value.float32, sizeof(kv->value.float32));
                break;
            case GGUF_TYPE_UINT64: gguf_bwrite_el(buf, &kv->value.uint64, sizeof(kv->value.uint64));
                break;
            case GGUF_TYPE_INT64: gguf_bwrite_el(buf, &kv->value.int64, sizeof(kv->value.int64));
                break;
            case GGUF_TYPE_FLOAT64: gguf_bwrite_el(buf, &kv->value.float64, sizeof(kv->value.float64));
                break;
            case GGUF_TYPE_BOOL: gguf_bwrite_el(buf, &kv->value.bool_, sizeof(kv->value.bool_));
                break;
            case GGUF_TYPE_STRING: gguf_bwrite_str(buf, &kv->value.str);
                break;
            case GGUF_TYPE_ARRAY: {
